...
use Pim\Component\Catalog\Event\FulfilledNewValueEvent;
...
class EntityWithValuesBuilder implements EntityWithValuesBuilderInterface
{
    ...
    public function addOrReplaceValue(
        ...
        // TODO: still not work for product creation as the id does not exists
        if (!$oldValue && $entityWithValues instanceof ProductInterface && $entityWithValues->getId() != null) {
            $entityWithValues->registerEvent(new FulfilledNewValueEvent($entityWithValues->getId(), $value));
        } else if (!$value->isEqual($oldValue) && $entityWithValues instanceof ProductInterface && $entityWithValues->getId() != null) {
            $entityWithValues->registerEvent(new FulfilledExistingValueEvent($entityWithValues->getId(), $value));
        }
